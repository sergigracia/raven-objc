...
- (void)captureException:(NSException *)exception sendNow:(BOOL)sendNow {
    ...
    NSDictionary *exceptionDict = [NSDictionary dictionaryWithObjectsAndKeys:
                                   exception.name, @"type",
                                   [self hideMemoryAdressesofString:exception.reason], @"value",
                                   nil];
...
- (NSString *)hideMemoryAdressesofString:(NSString *)message
{
    NSError *error;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"0x[a-z0-9]*" options:NSRegularExpressionCaseInsensitive error:&error];
    
    if (error) {
        NSLog(@"Error trying to remove memory adress from Raven Message: %@", error.description);
    }
    
    return [regex stringByReplacingMatchesInString:message options:nil range:NSMakeRange(0, [message length]) withTemplate:@"0x*********"];
}
